...
for root, dirs, files in os.walk(temp_folder_path):
    for file in files:
        ...
        try:
            ...
            print(f"{grn}[{folderfile_count}] Deleted file: \"{folder_file_path}\"{end}")
            folder_succes += 1

        except:
            print(f"{red}[{folderfile_count}] Error: cannot delete file: \"{folder_file_path}\" {end}")
            folder_errors += 1
...
for dir in dirs:
    try:
        ...
        print(f"{grn}[{dir_count}] Deleted folder: \"{folder_file_path}\{dir}\"{end}")
        dir_succes += 1

    except:
        print(f"{red}[{dir_count}] Cannot delete folder: \"{folder_file_path}\{dir}\"{end}")
        dir_errors += 1
